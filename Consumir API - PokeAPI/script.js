...
//carga pagina
window.loadPokeAPI = function() {
    const container = document.getElementById('poke-card');
    if (!container) return;
    container.textContent = 'Cargando pokemon...';
    fetchOnePokemon('pikachu')
        .then(data => renderPokemonNameImage(data, container))
        .catch(() => { container.textContent = 'Error cargando pokemon'; });
};